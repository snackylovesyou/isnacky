...
// Lista de respaldo automático de API Keys (rota automáticamente si una se agota)
...
async function searchYouTubeAPI(query: string): Promise<Song[]> {
  ...
  for (let attempt = 0; attempt < YOUTUBE_API_KEYS.length; attempt++) {
    ...
    try {
      ...
      if (data.error) {
        console.warn(`⚠️ La API Key en el índice [${currentApiKeyIndex}] llegó a su límite (429) o falló:`, data.error.message);
        currentApiKeyIndex = (currentApiKeyIndex + 1) % YOUTUBE_API_KEYS.length;
        console.log(`🔄 Cambiando automáticamente a la API Key alternativa [índice ${currentApiKeyIndex}]...`);
        continue;
      }
      ...
      console.error("Error de red con la llave activa, intentando la siguiente...", error);
      currentApiKeyIndex = (currentApiKeyIndex + 1) % YOUTUBE_API_KEYS.length;
    }
  }

  console.error("❌ Todas las API Keys registradas han agotado su cuota o fallaron.");
  return [];
}
...
function InicioContent({
  openOverlay, setCurrentSong, setPlaying, userFavorites, toggleFavorite, setQueue, recentSongs, profileImg, playlists, setActivePlaylistId
}: any) {
  ...
  return (
    <div className="flex flex-col overflow-y-auto flex-1 pb-4 tab-enter">
      <div className="flex items-center justify-between px-4 pt-12 pb-5">
        <div><p className="text-xs text-[#555] font-medium">Buenas noches</p><p className="text-2xl font-bold text-white tracking-tight">Snacky</p></div>
        <button onClick={() => openOverlay("perfil")} className="w-9 h-9 rounded-full overflow-hidden bg-gradient-to-br from-violet-500 to-pink-500 flex items-center justify-center text-white font-bold text-sm border border-white/10 shadow-lg btn-interactive">
          {profileImg ? <img src={profileImg} className="w-full h-full object-cover" /> : "MV"}
        </button>
      </div>
...
/* ── Formularios y Detalles de Playlists ────────────────── */
    ...
    <div className="flex flex-col h-full overflow-y-auto px-4 pt-12 pb-4 bg-[#080808] screen-enter">
      ...
      <div className="flex justify-center mb-8">
        <div className="relative w-32 h-32 rounded-2xl glass-dark flex items-center justify-center cursor-pointer group shadow-xl overflow-hidden btn-interactive" onClick={() => fileInputRefReal.current?.click()}>
          ...
          <input type="file" ref={fileInputRefReal} hidden accept="image/*" onChange={e => { if(e.target.files?.[0]) setImage(URL.createObjectURL(e.target.files[0])); }} />
        </div>
      </div>
...
/* ── Perfil Overlay ─────────────────────────────────────── */
function PerfilScreen({ onClose, profileImg, setProfileImg }: any) {
    ...
    <div className="flex flex-col h-full overflow-y-auto px-4 pt-12 pb-8 bg-[#080808] screen-enter">
      ...
      <div className="glass rounded-3xl p-5 flex items-center gap-4 mb-6">
        <div className="relative w-16 h-16 rounded-full bg-gradient-to-br from-violet-500 to-pink-500 flex items-center justify-center shadow-lg cursor-pointer group overflow-hidden btn-interactive" onClick={() => fileInputRefReal.current?.click()}>
          {profileImg ? <img src={profileImg} className="w-full h-full object-cover" /> : <span className="text-white font-bold text-xl">MV</span>}
          ...
          <input type="file" ref={fileInputRefReal} hidden accept="image/*" onChange={e => { if(e.target.files?.[0]) setProfileImg(URL.createObjectURL(e.target.files[0])); }} />
        </div>
        <div><p className="text-base font-bold text-white">Snacky</p><span className="text-[11px] font-semibold text-[#1ed760] bg-[#1ed760]/10 px-2 py-0.5 rounded-full mt-1 inline-block">Premium Individual</span></div>
      </div>
      <div className="mb-6"><SectionLabel>Configuración</SectionLabel><div className="glass rounded-3xl overflow-hidden divide-y divide-white/[0.06]"><button className="w-full flex items-center justify-between px-4 py-3.5 text-left btn-interactive"><span className="text-sm text-white">Calidad de audio</span><span className="text-xs text-[#555]">Muy alta</span></button><button className="w-full flex items-center justify-between px-4 py-3.5 text-left btn-interactive"><span className="text-sm text-red-400">Cerrar sesión</span></button></div></div>
...
export default function App() {
  ...
  const [userFavorites, setUserFavorites] = useState<Song[]>(initialLikedSongs);
      ...
      if (!window.ytPlayer) {
        window.ytPlayer = new window.YT.Player("youtube-player-root", {
          ...
          events: {
            ...
            onError: (e: any) => {
              console.warn("Error de reproducción detectado, reintentando...", e.data);
              setTimeout(() => {
      ...
      <div className="relative w-full max-w-[390px] flex flex-col overflow-hidden shadow-2xl" style={{ height: "100dvh", maxHeight: "844px" }}>
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          ...
          <div key={activeTab} className="relative z-10 flex-1 overflow-hidden flex flex-col">
            {activeTab === "inicio" && <InicioContent openOverlay={openOverlay} setCurrentSong={setCurrentSong} setPlaying={setPlaying} userFavorites={userFavorites} toggleFavorite={toggleFavorite} setQueue={setQueue} recentSongs={recentSongs} profileImg={profileImg} playlists={playlists} setActivePlaylistId={setActivePlaylistId} />}
          ...
          {overlay && (
            <div className="absolute inset-0 z-40 flex flex-col bg-[#080808]">
              {overlay === "perfil" && <PerfilScreen onClose={closeOverlay} profileImg={profileImg} setProfileImg={setProfileImg} />}
